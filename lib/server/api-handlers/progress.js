...
export default async function handler(req,res){try{const user=await requireUser(req,res);if(!user)return json(res,401,{ok:false,error:'Unauthorized'});const sql=sqlClient();
  ...
  if(req.method==='POST'){
    ...
    if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(lessonId))return json(res,400,{ok:false,error:'Invalid lesson_id'});
    ...
    const rawEid=String(b.event_id||'');const eid=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(rawEid)?rawEid:eventId();const submitted=String(b.answer||'').trim().toLowerCase().replace(/\s+/g,' ').slice(0,200);const direction=['en-ua','ua-en'].includes(b.direction)?b.direction:'en-ua';
    const result=await sql`
      ...
      answer_check AS (SELECT CASE WHEN ${submitted}='' OR NOT EXISTS(SELECT 1 FROM vocab) THEN false WHEN ${mode}='dictation' THEN regexp_replace(lower(trim((SELECT word FROM vocab))),'\s+',' ','g')=${submitted} WHEN ${mode}='match' THEN ${notion}=${submitted} WHEN ${direction}='en-ua' THEN regexp_replace(lower(trim((SELECT translation FROM vocab))),'\s+',' ','g')=${submitted} ELSE regexp_replace(lower(trim((SELECT word FROM vocab))),'\s+',' ','g')=${submitted} END AS correct),
      ...
      lesson_guard AS (SELECT id FROM lesson_sessions WHERE id=NULLIF(${lessonId},'')::uuid AND user_id=${user.id} AND mode=${mode} AND completed_at IS NULL AND answer_count < max_answers_per_session AND EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(lesson_words,'[]'::jsonb)) x WHERE x.value=${notion}) AND NOT EXISTS (SELECT 1 FROM lesson_attempts WHERE lesson_id=NULLIF(${lessonId},'')::uuid AND notion_id=${notion}) FOR UPDATE),
      ...
      inserted_event AS (
        ...
        FROM settings,user_before WHERE EXISTS(SELECT 1 FROM lesson_guard) AND NOT EXISTS(SELECT 1 FROM existing_event) ON CONFLICT(event_id) DO NOTHING RETURNING event_id,points
      ),
      card_upsert AS (
      ...
      SELECT (SELECT points FROM inserted_event LIMIT 1) AS points,(SELECT xp FROM user_update LIMIT 1) AS xp,(SELECT streak FROM user_update LIMIT 1) AS streak,(SELECT today_xp FROM user_update LIMIT 1) AS today_xp,(SELECT today FROM user_update LIMIT 1) AS today,(SELECT count(*) FROM existing_event)::int AS duplicate,(SELECT count(*) FROM inserted_event)::int AS inserted,(SELECT count(*) FROM lesson_guard)::int AS lesson_allowed,(SELECT row_to_json(card_upsert) FROM card_upsert LIMIT 1) AS card`;
    const r=result[0]||{};if(Number(r.lesson_allowed||0)===0)return json(res,409,{ok:false,error:'Lesson expired or answer limit reached'});if(Number(r.duplicate)>0||Number(r.inserted||0)===0)return json(res,200,{ok:true,duplicate:true,user:{xp:r.xp,streak:r.streak,todayXp:r.today_xp,today:r.today}});
