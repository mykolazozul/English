...
export default async function handler(req,res){try{const user=await requireUser(req,res);if(!user)return json(res,401,{ok:false,error:'Unauthorized'});const sql=sqlClient();
  ...
  if(req.method==='POST'){
    ...
    const result=await sql`
      ...
      answer_check AS (SELECT CASE WHEN ${submitted}='' OR NOT EXISTS(SELECT 1 FROM vocab) THEN false WHEN ${direction}='en-ua' THEN regexp_replace(lower(trim((SELECT translation FROM vocab))),'\s+',' ','g')=${submitted} ELSE regexp_replace(lower(trim((SELECT word FROM vocab))),'\s+',' ','g')=${submitted} END AS correct),
      ...
      lesson_guard AS (SELECT id FROM lesson_sessions WHERE id=NULLIF(${lessonId},'')::uuid AND user_id=${user.id} AND mode=${mode} AND completed_at IS NULL AND answer_count < max_answers_per_session AND EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(lesson_words,'[]'::jsonb)) x WHERE x.value=${notion}) FOR UPDATE),
