...
async function queryNotion(token, dataSourceId, cursor) {
  ...
  try {
    const r1 = await fetch(`https://api.notion.com/v1/data_sources/${dataSourceId}/query`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Notion-Version': API_VERSION, 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    });
    if (r1.ok) return await r1.json();
    if (r1.status !== 404 && r1.status !== 400) {
      const err = await r1.text();
      throw new Error(`Notion HTTP ${r1.status}: ${err.slice(0, 300)}`);
    }
  } catch (e) {
    if (!e.message.includes('404') && !e.message.includes('400')) throw e;
  }

  // Fallback to standard databases endpoint (v2022-06-28)
  ...
  if (!r2.ok) {
    ...
    throw new Error(`Notion API помилка (${r2.status}): ${err.slice(0, 300)}`);
  }
