...
async function queryNotion(token, dataSourceId, cursor) {
  ...
  if (!r2.ok) {
    ...
    if (r2.status === 404) {
      throw new Error(`Базу даних Notion (${activeId}) не знайдено або не підключено інтеграцію. Інструкція: відкрийте вашу базу слів у Notion -> натисніть '...' у правому верхньому кутку -> 'Add connections' ('Підключення') -> оберіть вашу інтеграцію 'New integration'. Додаток продовжує надійно працювати на повній збереженій базі з 300+ слів.`);
    }
