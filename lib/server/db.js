...
export function getIp(req){return String(req.headers?.['x-forwarded-for']||req.socket?.remoteAddress||'unknown').split(',')[0].trim().slice(0,100)}
...
export async function requireAdmin(req,res){const t=cookie(req,'__Host-ef_admin');if(!t)return null;const sql=sqlClient();const rows=await sql`SELECT u.id,u.nick,u.name,u.role FROM admin_sessions a JOIN users u ON u.role IN ('admin','moderator') WHERE a.token_hash=encode(digest(${t},'sha256'),'hex') AND a.expires_at>now() AND u.status='active' LIMIT 1`;if(!rows[0])return null;await sql`UPDATE admin_sessions SET last_seen_at=now() WHERE token_hash=encode(digest(${t},'sha256'),'hex')`;return rows[0];}
