...
const payload = `// AUTO-GENERATED by scripts/sync-notion.mjs — do not edit manually.\nexport const notionWords = ${JSON.stringify(all, null, 2)};\nexport const notionSyncMeta = ${JSON.stringify({ dataSourceId: DATA_SOURCE_ID, count: all.length, syncedAt: new Date().toISOString() }, null, 2)};\n`;

await fs.writeFile(path.resolve('src/notionWords.generated.js'), payload, 'utf8');
await fs.writeFile(path.resolve('public/notion-sync-meta.json'), JSON.stringify({ dataSourceId: DATA_SOURCE_ID, count: all.length, syncedAt: new Date().toISOString() }, null, 2), 'utf8');
