...
export default async function handler(req,res){try{const user=await requireUser(req,res);if(!user)return json(res,401,{ok:false,error:'Unauthorized'});const sql=sqlClient();
  ...
  if(req.method==='POST'){
    ...
    const eid=String(b.event_id||eventId()); const lessonId=String(b.lesson_id||'');let quality=Number(b.quality);if(!Number.isFinite(quality))quality=b.correct?4:1;quality=clamp(Math.round(quality),0,5);const correct=quality>=3;
    const result=await sql`
      ...
      vocab AS (SELECT notion_id FROM vocabulary WHERE notion_id=${notion} LIMIT 1),
      ...
      next_srs AS (
        SELECT CASE WHEN ${correct} THEN jsonb_build_object(
          'version',2,
          'stage',LEAST(8,stage+1),
          'ease',ROUND(GREATEST(1.3,ease + CASE WHEN ${quality}=5 THEN 0.10 WHEN ${quality}=3 THEN -0.15 ELSE 0 END)::numeric,2),
          'interval',CASE WHEN reps=0 THEN 1 WHEN reps=1 THEN 3 ELSE GREATEST(1,ROUND(interval * GREATEST(1.3,ease + CASE WHEN ${quality}=5 THEN 0.10 WHEN ${quality}=3 THEN -0.15 ELSE 0 END))) END,
          'dueAt',(current_date + (CASE WHEN reps=0 THEN 1 WHEN reps=1 THEN 3 ELSE GREATEST(1,ROUND(interval * GREATEST(1.3,ease + CASE WHEN ${quality}=5 THEN 0.10 WHEN ${quality}=3 THEN -0.15 ELSE 0 END))) END)::int),
          'repetitions',reps+1,
          'lapses',lapses
      ...
      card_upsert AS (
        ...
        SELECT ${user.id},${notion},CASE WHEN ${correct} THEN 1 ELSE 0 END,1,CASE WHEN ${correct} THEN 1 ELSE 0 END,CASE WHEN ${correct} THEN 0 ELSE 1 END,next_srs.srs,now()
        FROM next_srs WHERE EXISTS(SELECT 1 FROM vocab) AND EXISTS(SELECT 1 FROM inserted_event)
        ON CONFLICT(user_id,notion_id) DO UPDATE SET mastery=GREATEST(0,user_vocabulary.mastery+CASE WHEN ${correct} THEN 1 ELSE 0 END),attempts=user_vocabulary.attempts+1,correct=user_vocabulary.correct+CASE WHEN ${correct} THEN 1 ELSE 0 END,wrong=user_vocabulary.wrong+CASE WHEN ${correct} THEN 0 ELSE 1 END,srs=excluded.srs,last_answered_at=now()
      ...
      attempt_insert AS (
        ...
        SELECT ${eid},NULLIF(${lessonId},'')::uuid,${user.id},(SELECT notion_id FROM vocab),${mode},${correct},points,${quality} FROM inserted_event RETURNING points
      ),
      lesson_update AS (UPDATE lesson_sessions SET answer_count=answer_count+1,correct_count=correct_count+CASE WHEN ${correct} THEN 1 ELSE 0 END WHERE id=NULLIF(${lessonId},'')::uuid AND user_id=${user.id} AND EXISTS(SELECT 1 FROM inserted_event) RETURNING id),
      user_update AS (
        UPDATE users SET
      ...
      SELECT (SELECT points FROM inserted_event LIMIT 1) AS points,(SELECT xp FROM user_update LIMIT 1) AS xp,(SELECT streak FROM user_update LIMIT 1) AS streak,(SELECT today_xp FROM user_update LIMIT 1) AS today_xp,(SELECT today FROM user_update LIMIT 1) AS today,(SELECT count(*) FROM existing_event)::int AS duplicate`;
