...
async function api(path,options={}){const res=await fetch(path,{credentials:'include',headers:{'Content-Type':'application/json',...(options.headers||{})},...options});const data=await res.json().catch(()=>({}));if(!res.ok){const e=new Error(data.error||`HTTP ${res.status}`);e.status=res.status;e.data=data;const isEx=String(path).startsWith('/api/auth')||String(path).startsWith('/api/admin')||String(path).startsWith('/api/reports')||String(path).startsWith('/api/notion-sync')||String(path).startsWith('/api/analytics')||String(path).startsWith('/api/gamification');if(res.status===401&&!isEx)window.dispatchEvent(new CustomEvent('ef-auth-expired',{detail:{path,message:e.message}}));throw e}return data}
